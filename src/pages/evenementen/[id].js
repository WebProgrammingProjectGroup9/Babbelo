...
export default function Detail() {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        ...
        const userJoined = eventData.participants.some(
  ...
  const handleJoin = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/event/${id}/join`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="card h-100 bg-light shadow-lg p-3 detail-card">
      ...
      <ul className="list-unstyled">
        ...
        <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
          ...
          <span>
            {new Date(`2025-01-01T${event.startTime}`).toLocaleTimeString(
              "nl-NL",
        ...
        <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
          ...
          <span>
            {new Date(`2025-01-01T${event.endTime}`).toLocaleTimeString(
              "nl-NL",
              { hour: "2-digit", minute: "2-digit" }
            )}
          </span>
...
<div className="row mt-5 mb-5">
    ...
    <div className="col-6">
      <div className="card shadow-lg h-100">
        ...
        <div className="card-body">
          <Link
            href={`/account/${event.organisator.id}`}
            ...
            <div className="d-flex align-items-center p-1 mb-4">
              ...
              <div className="ps-2">
                ...
                <p className="mb-0 text-muted">
                  ...
                  {event.organisator.emailAddress ||
                    "Geen e-mailadres beschikbaar"}
                </p>
                <p className="mb-0 text-muted">
                  <i className="bi bi-telephone me-2"></i>
                  {event.organisator.phoneNumber ||
  ...
  {event.address && (
    <div className="col-6">
      <div className="card shadow-lg h-100">
        <div className="card-header bg-white">
          <h3 className="display-6 text-center">Locatie</h3>
        </div>
        <div className="card-body">
          <ul className="list-unstyled">
            <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
              <strong>
                <i className="bi bi-geo-alt me-2"></i>Postcode:
              </strong>
              <span>{event.address.zipCode || "Niet beschikbaar"}</span>
            </li>
            <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
              <strong>
                <i className="bi bi-building me-2"></i>Stad:
              </strong>
              <span>{event.address.city || "Niet beschikbaar"}</span>
            </li>
            <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
              <strong>
                <i className="bi bi-house me-2"></i>Straat:
              </strong>
              <span>{event.address.streetName || "Niet beschikbaar"}</span>
            </li>
            <li className="mb-3 border-bottom pb-1 d-flex justify-content-between">
              <strong>
                <i className="bi bi-door-closed me-2"></i>Huisnummer:
              </strong>
              <span>{event.address.houseNumber || "Niet beschikbaar"}</span>
            </li>
          </ul>
        </div>
      </div>
